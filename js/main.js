/**
 * Application entry point - connects Camera, Canvas, PointManager, UI, and Sensors.
 * Supports: Snapshot mode (all devices), WebXR AR (Android+ARCore).
 * Direction modes: Sensor auto (compass/gyro/accel) or Manual (preset buttons).
 * @module main
 */

import CameraManager from './modules/CameraManager.js';
import CanvasOverlay from './modules/CanvasOverlay.js';
import PointManager from './modules/PointManager.js';
import UIController from './modules/UIController.js';
import ProjectStorage from './modules/ProjectStorage.js';
import CSVExporter from './modules/CSVExporter.js';
import DXFExporter from './modules/DXFExporter.js';
import { $ } from './utils/dom.js';

class App {
  constructor() {
    this.pointManager = new PointManager();
    this.uiController = new UIController(this.pointManager);
    this.camera = new CameraManager($('camera-video'));
    this.canvas = new CanvasOverlay($('camera-overlay'), (x, y) => this._onTap(x, y));
    this.storage = new ProjectStorage();
    this.csvExporter = new CSVExporter();
    this.dxfExporter = new DXFExporter();

    this._projectName = '';
    this._metadata = { siteName: '', operator: '', pipeType: '' };
    this._mode = 'snapshot'; // 'snapshot' or 'ar'
    this._xrManager = null;
    this._arSupported = false;

    // AR calibration state
    this._arCalibrationMode = false;
    this._arCalibrationCount = 0; // 0, 1, or 2 calibration anchors placed
    this._arScaleFactor = 1.0;
    this._arCalibrationResolve = null;
    this._arActionsBound = false;
    this._arShowLabels = true;

    // Direction / sensor state
    this._directionMode = 'none'; // 'sensor' | 'manual' | 'none'
    this._directionModeSelected = false;
    this._sensorManager = null;
    this._compassUI = null;
    this._lastHeading = 0;       // running heading for manual mode

    // Distance mode: 'manual' (dialog each tap) or 'auto' (pixel calibration)
    this._distanceMode = 'manual';

    // Track which mode was used for recording (for "return to camera" after AR)
    this._returnMode = 'snapshot'; // 'snapshot' or 'ar'

    // Canvas overlay always updates
    this.pointManager.onChange((points) => {
      this.canvas.setPoints(points);
      if (this.viewer3D) {
        try { this.viewer3D.updateRoute(points); }
        catch (err) { console.error('3D updateRoute error:', err); }
      }
    });

    this._bindActions();
    this._selectMode();
    this._init3D();
  }

  // ─── Mode Selection ───────────────────────────────────────

  async _selectMode() {
    try {
      const { default: WebXRManager } = await import('./modules/WebXRManager.js');
      this._arSupported = await WebXRManager.isSupported();
    } catch { this._arSupported = false; }

    if (this._arSupported) {
      $('modal-mode-select').hidden = false;
    } else {
      this._startSnapshotMode();
    }
  }

  async _startSnapshotMode() {
    this._mode = 'snapshot';
    this._returnMode = 'snapshot';
    $('section-camera').hidden = false;
    await this._startCamera();
    // Show direction mode dialog once, then distance mode dialog
    if (!this._directionModeSelected) {
      await this._showDirectionModeDialog();
      this._directionModeSelected = true;
      await this._showDistanceModeDialog();
    }
    console.log('Mode: Snapshot, Direction:', this._directionMode,
      'Distance:', this._distanceMode);
  }

  // ─── Direction Mode Selection ─────────────────────────────

  _showDirectionModeDialog() {
    return new Promise((resolve) => {
      const dialog = $('modal-direction-mode');
      dialog.hidden = false;

      const onSensor = async () => {
        dialog.hidden = true;
        cleanup();
        await this._initSensors();
        resolve();
      };

      const onManual = () => {
        dialog.hidden = true;
        cleanup();
        this._directionMode = 'manual';
        resolve();
      };

      $('btn-direction-sensor').addEventListener('click', onSensor);
      $('btn-direction-manual').addEventListener('click', onManual);

      function cleanup() {
        $('btn-direction-sensor').removeEventListener('click', onSensor);
        $('btn-direction-manual').removeEventListener('click', onManual);
      }
    });
  }

  // ─── Distance Mode Selection ─────────────────────────────

  _showDistanceModeDialog() {
    return new Promise((resolve) => {
      const dialog = $('modal-distance-mode');
      dialog.hidden = false;

      const onManual = () => {
        dialog.hidden = true;
        cleanup();
        this._distanceMode = 'manual';
        resolve();
      };

      const onAuto = () => {
        dialog.hidden = true;
        cleanup();
        this._distanceMode = 'auto';
        resolve();
      };

      $('btn-distance-mode-manual').addEventListener('click', onManual);
      $('btn-distance-mode-auto').addEventListener('click', onAuto);

      function cleanup() {
        $('btn-distance-mode-manual').removeEventListener('click', onManual);
        $('btn-distance-mode-auto').removeEventListener('click', onAuto);
      }
    });
  }

  // ─── Sensor Initialization ────────────────────────────────

  async _initSensors() {
    try {
      const { default: SensorManager } = await import('./modules/SensorManager.js');
      const { default: CompassUI } = await import('./modules/CompassUI.js');

      this._sensorManager = new SensorManager();
      this._compassUI = new CompassUI();

      // init() triggers iOS permission (called from user-gesture context)
      const info = await this._sensorManager.init();
      console.log('Sensors:', info.description);

      if (info.level >= 5) {
        alert('\u30BB\u30F3\u30B5\u30FC\u304C\u5229\u7528\u3067\u304D\u307E\u305B\u3093\u3002\u624B\u52D5\u5165\u529B\u30E2\u30FC\u30C9\u3067\u52D5\u4F5C\u3057\u307E\u3059\u3002');
        this._directionMode = 'manual';
        return;
      }

      this._directionMode = 'sensor';

      // Level 3 (gyro only, no compass): prompt initial heading
      if (info.level === 3) {
        const heading = await this._compassUI.promptInitialHeading();
        this._sensorManager.setInitialHeading(heading);
        this._lastHeading = heading;
      }

      // Start live display
      this._sensorManager.onChange((data) => {
        this._compassUI.update(data.heading, data.elevation, data.level,
          this._sensorManager._compassAccuracy);
      });

      // Show initial reading
      const r = this._sensorManager.captureReading();
      this._compassUI.update(r.heading, r.elevation, r.level, r.accuracy);

    } catch (err) {
      console.error('Sensor init failed:', err);
      alert('\u30BB\u30F3\u30B5\u30FC\u521D\u671F\u5316\u5931\u6557\u3002\u624B\u52D5\u5165\u529B\u30E2\u30FC\u30C9\u3067\u52D5\u4F5C\u3057\u307E\u3059\u3002');
      this._directionMode = 'manual';
    }
  }

  // ─── AR Mode ──────────────────────────────────────────────

  async _startARMode() {
    this._mode = 'ar';
    this._returnMode = 'ar';
    this._arCalibrationMode = false;
    this._arCalibrationCount = 0;
    this._arScaleFactor = 1.0;
    try {
      const { default: WebXRManager } = await import('./modules/WebXRManager.js');
      this._xrManager = new WebXRManager((anchorIdx) => this._onARTap(anchorIdx));
      $('app-main').hidden = true;
      $('app-header').hidden = true;
      const arOverlay = $('ar-overlay');
      arOverlay.hidden = false;
      $('ar-status').textContent = '\u9762\u3092\u30BF\u30C3\u30D7\u3057\u3066\u30A2\u30F3\u30AB\u30FC\u3092\u914D\u7F6E';
      $('ar-point-count').textContent = '0\u70B9';
      $('btn-ar-calibrate').disabled = false;
      $('btn-ar-calibrate').classList.remove('active');
      $('btn-ar-labels').textContent = this._arShowLabels ? '距離:ON' : '距離:OFF';
      $('btn-ar-labels').classList.toggle('active', this._arShowLabels);
      this._bindARActions();
      const result = await this._xrManager.start(arOverlay);
      if (!result.success) { alert(result.error); this._exitARMode(); return; }
      console.log('Mode: WebXR AR');
    } catch (err) {
      alert(`AR\u30E2\u30FC\u30C9\u306E\u8D77\u52D5\u306B\u5931\u6557: ${err.message}`);
      this._exitARMode();
    }
  }

  _exitARMode() {
    // Cancel any open calibration
    if (this._arCalibrationResolve) {
      this._resolveARCalibration({ cancelled: true });
    }
    this._arCalibrationMode = false;
    this._arCalibrationCount = 0;

    if (this._xrManager) {
      this._xrManager.stop();
      this._xrManager.dispose();
      this._xrManager = null;
    }
    $('ar-overlay').hidden = true;
    $('app-main').hidden = false;
    $('app-header').hidden = false;
    this._mode = 'snapshot';

    // Go directly to result view (not camera)
    $('section-camera').hidden = true;
    $('bar-return-camera').hidden = false;

    if (this.viewer3D) {
      this.viewer3D.forceResize();
      if (this.pointManager.points.length > 0) {
        this.viewer3D.updateRoute(this.pointManager.points);
      }
    }
  }

  // ─── AR Mode Handlers ─────────────────────────────────────

  _bindARActions() {
    if (this._arActionsBound) return;
    this._arActionsBound = true;

    // ① Undo: cancel calibration or undo last recording point
    $('btn-ar-undo').addEventListener('click', () => {
      if (!this._xrManager) return;
      this._suppressARSelect();

      // Cancel calibration dialog if open
      if (this._arCalibrationResolve) {
        this._resolveARCalibration({ cancelled: true });
      }

      // If in calibration mode, exit and undo calibration anchors
      if (this._arCalibrationMode) {
        this._exitCalibrationMode();
        return;
      }

      // Normal undo
      this._xrManager.undoLastAnchor();
      this.pointManager.undoLastPoint();
      $('ar-point-count').textContent = `${this.pointManager.getCount()}\u70B9`;
    });

    // ② Done: exit to result view
    $('btn-ar-done').addEventListener('click', () => {
      this._suppressARSelect();
      this._exitARMode();
    });

    // ④ Calibrate button
    $('btn-ar-calibrate').addEventListener('click', () => {
      if (!this._xrManager) return;
      this._suppressARSelect();
      if (this._arCalibrationMode) return;
      this._enterCalibrationMode();
    });

    // ③ Clear all: remove all anchors and points
    $('btn-ar-clear').addEventListener('click', () => {
      if (!this._xrManager) return;
      this._suppressARSelect();
      if (this.pointManager.getCount() === 0) return;
      if (!confirm('すべてのポイントをクリアしますか？')) return;

      if (this._arCalibrationResolve) {
        this._resolveARCalibration({ cancelled: true });
      }
      if (this._arCalibrationMode) {
        this._arCalibrationMode = false;
        this._arCalibrationCount = 0;
        $('btn-ar-calibrate').classList.remove('active');
        $('btn-ar-calibrate').disabled = false;
      }
      this._xrManager.clearAllAnchors();
      this.pointManager.clear();
      $('ar-point-count').textContent = '0\u70B9';
      const scaleText = this._arScaleFactor !== 1.0
        ? ` (\u6821\u6B63\u6E08: \u00D7${this._arScaleFactor.toFixed(2)})` : '';
      $('ar-status').textContent = `\u9762\u3092\u30BF\u30C3\u30D7\u3057\u3066\u30A2\u30F3\u30AB\u30FC\u3092\u914D\u7F6E${scaleText}`;
    });

    // ② Distance label toggle
    $('btn-ar-labels').addEventListener('click', () => {
      if (!this._xrManager) return;
      this._suppressARSelect();
      this._arShowLabels = !this._arShowLabels;
      this._xrManager.showDistanceLabels = this._arShowLabels;
      $('btn-ar-labels').textContent = this._arShowLabels ? '\u8DDD\u96E2:ON' : '\u8DDD\u96E2:OFF';
      $('btn-ar-labels').classList.toggle('active', this._arShowLabels);
    });

    // Calibration dialog buttons
    $('btn-ar-calibration-ok').addEventListener('click', () => {
      this._suppressARSelect();
      const rawVal = $('ar-input-calibration').value.trim();
      if (rawVal === '') return;
      const distance = parseFloat(rawVal);
      if (isNaN(distance) || distance <= 0) {
        alert('\u6709\u52B9\u306A\u8DDD\u96E2\u3092\u5165\u529B\u3057\u3066\u304F\u3060\u3055\u3044');
        return;
      }
      this._resolveARCalibration({ distance, cancelled: false });
    });
    $('btn-ar-calibration-cancel').addEventListener('click', () => {
      this._suppressARSelect();
      this._resolveARCalibration({ cancelled: true });
    });
  }

  /** Temporarily suppress XR select to prevent simultaneous tap-through */
  _suppressARSelect() {
    if (!this._xrManager) return;
    this._xrManager.suppressSelect = true;
    clearTimeout(this._arSuppressTimer);
    this._arSuppressTimer = setTimeout(() => {
      if (this._xrManager) this._xrManager.suppressSelect = false;
    }, 300);
  }

  // ─── AR Recording (no dialog per tap) ──────────────────────

  async _onARTap(anchorIdx) {
    // ④ Calibration mode: taps go to calibration, not recording
    if (this._arCalibrationMode) {
      this._handleCalibrationTap(anchorIdx);
      return;
    }

    // Normal recording: add point + auto-set AR distance
    const addResult = this.pointManager.addPoint(0, 0);
    if (!addResult.success) return;
    $('ar-point-count').textContent = `${this.pointManager.getCount()}\u70B9`;

    if (anchorIdx >= 1) {
      const arSegIdx = anchorIdx - 1;
      const measured = this._xrManager.getAnchorDistance(arSegIdx);
      const dist = measured !== null ? measured * this._arScaleFactor : null;
      const pmSegIdx = this.pointManager.getCount() - 2;
      if (dist !== null && pmSegIdx >= 0) {
        this.pointManager.setSegmentDistance(pmSegIdx, dist);
      }
      // ① Derive direction from AR anchor 3D coordinates
      const dir = this._xrManager.getAnchorDirection(arSegIdx);
      if (dir && pmSegIdx >= 0) {
        this.pointManager.setPointDirection(
          pmSegIdx, dir.heading, dir.elevation, 'ar', 1
        );
      }
    }
  }

  // ─── AR Calibration ────────────────────────────────────────

  _enterCalibrationMode() {
    this._arCalibrationMode = true;
    this._arCalibrationCount = 0;
    $('ar-status').textContent = '\u6821\u6B63: \u57FA\u6E961\u70B9\u76EE\u3092\u30BF\u30C3\u30D7';
    $('btn-ar-calibrate').classList.add('active');
    $('btn-ar-calibrate').disabled = true;
  }

  _exitCalibrationMode() {
    // Remove calibration anchors from WebXR (they are at the end of anchor list)
    for (let i = 0; i < this._arCalibrationCount; i++) {
      this._xrManager.undoLastAnchor();
    }
    this._arCalibrationMode = false;
    this._arCalibrationCount = 0;
    $('ar-calibration-dialog').hidden = true;
    $('btn-ar-calibrate').classList.remove('active');
    $('btn-ar-calibrate').disabled = false;
    $('ar-point-count').textContent = `${this.pointManager.getCount()}\u70B9`;

    const scaleText = this._arScaleFactor !== 1.0
      ? ` (\u6821\u6B63\u6E08: \u00D7${this._arScaleFactor.toFixed(2)})` : '';
    $('ar-status').textContent = `\u9762\u3092\u30BF\u30C3\u30D7\u3057\u3066\u30A2\u30F3\u30AB\u30FC\u3092\u914D\u7F6E${scaleText}`;
  }

  async _handleCalibrationTap(anchorIdx) {
    this._arCalibrationCount++;

    if (this._arCalibrationCount === 1) {
      $('ar-status').textContent = '\u6821\u6B63: \u57FA\u6E962\u70B9\u76EE\u3092\u30BF\u30C3\u30D7';
      $('ar-point-count').textContent = '\u6821\u6B63\u4E2D...';
    } else if (this._arCalibrationCount === 2) {
      // ③ Suppress taps during dialog
      this._xrManager.suppressSelect = true;

      const arDist = this._xrManager.getAnchorDistance(anchorIdx - 1);
      $('ar-calibration-measured').textContent = arDist !== null
        ? `AR\u8A08\u6E2C: ${arDist.toFixed(3)}m` : 'AR\u8A08\u6E2C: --';
      $('ar-input-calibration').value = '';
      $('ar-calibration-dialog').hidden = false;

      const result = await this._promptARCalibration();

      this._xrManager.suppressSelect = false;

      if (!result.cancelled && result.distance != null && arDist > 0) {
        this._arScaleFactor = result.distance / arDist;
        this._rescaleARDistances();
      }

      this._exitCalibrationMode();
    }
  }

  _promptARCalibration() {
    return new Promise((resolve) => {
      this._arCalibrationResolve = resolve;
    });
  }

  _resolveARCalibration(result) {
    $('ar-calibration-dialog').hidden = true;
    if (this._arCalibrationResolve) {
      this._arCalibrationResolve(result);
      this._arCalibrationResolve = null;
    }
  }

  /** Re-read AR distances and apply current scale factor */
  _rescaleARDistances() {
    const count = this.pointManager.getCount();
    for (let i = 0; i < count - 1; i++) {
      const arDist = this._xrManager.getAnchorDistance(i);
      if (arDist !== null) {
        this.pointManager.setSegmentDistance(i, arDist * this._arScaleFactor);
      }
    }
    // Update distance labels in AR view
    if (this._xrManager) {
      this._xrManager.scaleFactor = this._arScaleFactor;
    }
  }

  // ─── 3D Viewer ────────────────────────────────────────────

  async _init3D() {
    const container = $('viewer-container');
    const statusEl = document.createElement('p');
    statusEl.id = 'viewer-status';
    statusEl.style.cssText = 'color:#888;text-align:center;padding:16px;margin:0;';
    statusEl.textContent = '3D\u30D7\u30EC\u30D3\u30E5\u30FC\u3092\u8AAD\u307F\u8FBC\u307F\u4E2D...';
    container.appendChild(statusEl);

    try {
      const { default: Viewer3D } = await import('./modules/Viewer3D.js');
      this.viewer3D = new Viewer3D('viewer-container');
      if (statusEl.parentElement) statusEl.remove();

      const { default: GLBExporter } = await import('./modules/GLBExporter.js');
      const { default: OBJExporter } = await import('./modules/OBJExporter.js');
      this.glbExporter = new GLBExporter();
      this.objExporter = new OBJExporter();

      $('btn-export-glb').addEventListener('click', () => {
        this.glbExporter.export(this.viewer3D.getScene(), this.pointManager.points);
      });
      $('btn-export-obj').addEventListener('click', () => {
        this.objExporter.export(this.pointManager.points);
      });
      document.querySelectorAll('.btn-view').forEach(btn => {
        if (btn.dataset.view) {
          btn.addEventListener('click', () => this.viewer3D.setView(btn.dataset.view));
        }
      });

      // Origin toggle (start point / end point)
      const originBtn = $('btn-origin-toggle');
      if (originBtn) {
        originBtn.addEventListener('click', () => {
          const mode = this.viewer3D.toggleOrigin();
          originBtn.textContent = mode === 'start' ? '\u539F\u70B9:\u59CB\u70B9' : '\u539F\u70B9:\u7D42\u70B9';
          if (this.pointManager.points.length > 0) {
            this.viewer3D.updateRoute(this.pointManager.points);
          }
        });
      }

      // ── Display mode toggle (wire / pipe) ──
      $('btn-display-wire').addEventListener('click', () => {
        this.viewer3D.setDisplayMode('wire');
        $('btn-display-wire').classList.add('active');
        $('btn-display-pipe').classList.remove('active');
        $('input-pipe-radius').disabled = true;
      });
      $('btn-display-pipe').addEventListener('click', () => {
        this.viewer3D.setDisplayMode('pipe');
        $('btn-display-pipe').classList.add('active');
        $('btn-display-wire').classList.remove('active');
        $('input-pipe-radius').disabled = false;
      });
      $('input-pipe-radius').addEventListener('change', (e) => {
        const mm = parseFloat(e.target.value);
        if (!isNaN(mm) && mm > 0) {
          this.viewer3D.setPipeRadius(mm / 1000); // mm → m
        }
      });

      // ── Distance labels toggle ──
      const labelBtn = $('btn-toggle-labels');
      labelBtn.addEventListener('click', () => {
        const showing = labelBtn.textContent === '表示中';
        this.viewer3D.setShowLabels(!showing);
        labelBtn.textContent = showing ? '非表示' : '表示中';
        labelBtn.classList.toggle('active', !showing);
      });

      // ── Grid size selector ──
      $('select-grid-size').addEventListener('change', (e) => {
        this.viewer3D.setGridSize(parseInt(e.target.value, 10));
      });

      if (this.pointManager.points.length > 0) {
        this.viewer3D.updateRoute(this.pointManager.points);
      }

      console.log('3D viewer initialized (importmap)');
    } catch (err) {
      console.error('3D viewer init failed:', err);
      statusEl.textContent = `3D\u30D7\u30EC\u30D3\u30E5\u30FC: \u521D\u671F\u5316\u30A8\u30E9\u30FC (${err.message})`;
      statusEl.style.color = '#f44336';
    }
  }

  // ─── Camera (Snapshot Mode) ───────────────────────────────

  async _startCamera() {
    const errorEl = $('camera-error');
    const msgEl = $('camera-error-msg');
    msgEl.textContent = '\u30AB\u30E1\u30E9\u3092\u8D77\u52D5\u4E2D...';
    errorEl.hidden = false;
    const result = await this.camera.start();
    if (result.success) {
      errorEl.hidden = true;
      this.canvas.syncSize();
    } else {
      msgEl.textContent = result.error;
      errorEl.hidden = false;
    }
  }

  // ─── Tap Handler ─────────────────────────────────────────

  async _onTap(screenX, screenY) {
    // Capture sensor reading at tap time (live video, no freeze)
    let tapReading = null;
    if (this._directionMode === 'sensor' && this._sensorManager) {
      tapReading = this._sensorManager.captureReading();
    }

    const prevCount = this.pointManager.getCount();
    const addResult = this.pointManager.addPoint(screenX, screenY);
    if (!addResult.success) {
      if (addResult.errors?.count) alert(addResult.errors.count);
      return;
    }

    if (prevCount >= 1) {
      const segIndex = prevCount - 1;
      const isFirst = segIndex === 0 && this.pointManager.calibration.pixelsPerMeter === null;

      if (this._distanceMode === 'auto' && !isFirst) {
        // Auto mode: estimate distance from pixel positions
        const estimated = this.pointManager.estimateDistance(segIndex);
        if (estimated !== null) {
          this.pointManager.setSegmentDistance(segIndex, estimated);
        }
        // Set direction from sensor reading
        this._applyTapDirection(segIndex, tapReading);
      } else {
        // Manual mode (or first segment needing calibration)
        const dirOptions = this._buildDirOptions(tapReading);
        const result = await this.uiController.promptDistance(segIndex, isFirst, dirOptions);

        if (result.distance !== null) {
          this.pointManager.setSegmentDistance(segIndex, result.distance);
          if (isFirst) this.pointManager.calibrate(segIndex, result.distance);
        }

        if (result.heading != null) {
          this.pointManager.setPointDirection(
            segIndex, result.heading, result.elevation || 0,
            result.directionSource || 'manual',
            this._sensorManager?.level || 5
          );
          this._lastHeading = result.heading;
        }

        if (result.memo) {
          this.pointManager.updateMemo(addResult.point.id, result.memo);
        }
      }
    }

    this._updateCameraPointCount();
  }

  /** Applies sensor direction to a segment (auto distance mode) */
  _applyTapDirection(segIndex, tapReading) {
    if (tapReading && tapReading.heading != null) {
      this.pointManager.setPointDirection(
        segIndex, tapReading.heading, tapReading.elevation || 0,
        tapReading.source || 'sensor', tapReading.level || 5
      );
      this._lastHeading = tapReading.heading;
    } else if (this._directionMode === 'manual') {
      // Manual mode: default to straight
      const heading = this._lastHeading;
      this.pointManager.setPointDirection(segIndex, heading, 0, 'manual', 5);
    }
  }

  _buildDirOptions(tapReading) {
    if (this._directionMode === 'sensor') {
      const reading = tapReading || this._sensorManager?.captureReading();
      return { mode: 'sensor', reading };
    }
    if (this._directionMode === 'manual') {
      return { mode: 'manual', prevHeading: this._lastHeading };
    }
    return null;
  }

  _updateCameraPointCount() {
    const el = $('camera-point-count');
    if (el) el.textContent = `${this.pointManager.getCount()}\u70B9`;
  }

  // ─── Global Button Actions ────────────────────────────────

  _bindActions() {
    $('btn-camera-toggle').addEventListener('click', async () => {
      const result = await this.camera.toggleCamera();
      if (!result.success) alert(result.error);
    });

    $('btn-camera-retry').addEventListener('click', () => this._startCamera());

    $('btn-camera-done').addEventListener('click', () => this._exitCameraMode());
    $('btn-return-camera').addEventListener('click', () => this._enterCameraMode());

    $('btn-undo').addEventListener('click', () => this.pointManager.undoLastPoint());
    $('btn-calibrate').addEventListener('click', () => this._recalibrate());

    // Clear all recorded points (result view)
    $('btn-clear-all-points').addEventListener('click', () => {
      if (this.pointManager.getCount() === 0) return;
      if (!confirm('すべての記録済みポイントを削除しますか？')) return;
      this.pointManager.clear();
    });

    // ⑥ Mode reset button: go back to mode selection
    $('btn-mode-reset').addEventListener('click', () => this._resetToModeSelect());

    // Mode selection buttons (bound once, reusable on reset)
    $('btn-mode-snapshot').addEventListener('click', () => {
      $('modal-mode-select').hidden = true;
      this._startSnapshotMode();
    });
    $('btn-mode-ar').addEventListener('click', () => {
      $('modal-mode-select').hidden = true;
      this._startARMode();
    });

    $('btn-save').addEventListener('click', () => this._saveProject());
    $('btn-load').addEventListener('click', () => this._loadProject());
    $('btn-new').addEventListener('click', () => this._newProject());

    $('btn-settings').addEventListener('click', () => this._openSettings());
    $('btn-settings-close').addEventListener('click', () => this._closeSettings());
    $('form-settings').addEventListener('submit', (e) => {
      e.preventDefault();
      this._saveSettings();
    });
    $('modal-settings').addEventListener('click', (e) => {
      if (e.target.id === 'modal-settings') this._closeSettings();
    });

    $('btn-export-csv').addEventListener('click', () => {
      this.csvExporter.export(this.pointManager.points);
    });
    $('btn-export-dxf').addEventListener('click', () => {
      this.dxfExporter.export(this.pointManager.points);
    });
  }

  // ─── Camera Mode Transitions ─────────────────────────────

  _exitCameraMode() {
    this.camera.stop();
    $('section-camera').hidden = true;
    $('bar-return-camera').hidden = false;
    // Force 3D viewer resize after becoming visible
    if (this.viewer3D) {
      this.viewer3D.forceResize();
      if (this.pointManager.points.length > 0) {
        this.viewer3D.updateRoute(this.pointManager.points);
      }
    }
  }

  async _enterCameraMode() {
    if (this._returnMode === 'ar' && this._arSupported) {
      $('bar-return-camera').hidden = true;
      this._startARMode();
      return;
    }
    $('bar-return-camera').hidden = true;
    $('section-camera').hidden = false;
    await this._startCamera();
    this.canvas.syncSize();
  }

  /** ⑥ Reset to mode selection screen */
  _resetToModeSelect() {
    this.camera.stop();
    $('section-camera').hidden = true;
    $('bar-return-camera').hidden = true;
    this._directionModeSelected = false;
    this._directionMode = 'none';
    this._distanceMode = 'manual';
    this._sensorManager = null;
    this._compassUI = null;

    if (this._arSupported) {
      $('modal-mode-select').hidden = false;
    } else {
      this._startSnapshotMode();
    }
  }

  async _recalibrate() {
    if (this.pointManager.getCount() < 2) {
      alert('\u6821\u6B63\u306B\u306F2\u70B9\u4EE5\u4E0A\u304C\u5FC5\u8981\u3067\u3059');
      return;
    }
    const result = await this.uiController.promptDistance(0, true);
    if (result.distance !== null) {
      this.pointManager.calibrate(0, result.distance);
      this.pointManager.setSegmentDistance(0, result.distance);
    }
  }

  _openSettings() {
    $('input-project-name').value = this._projectName;
    $('input-site-name').value = this._metadata.siteName;
    $('input-operator').value = this._metadata.operator;
    $('input-pipe-type').value = this._metadata.pipeType;
    $('modal-settings').hidden = false;
  }

  _closeSettings() { $('modal-settings').hidden = true; }

  _saveSettings() {
    this._projectName = $('input-project-name').value.trim();
    this._metadata = {
      siteName: $('input-site-name').value.trim(),
      operator: $('input-operator').value.trim(),
      pipeType: $('input-pipe-type').value.trim()
    };
    this._closeSettings();
  }

  _saveProject() {
    const result = this.storage.save(
      this._projectName, this._metadata,
      this.pointManager.points, this.pointManager.calibration
    );
    alert(result.success ? '\u4FDD\u5B58\u3057\u307E\u3057\u305F' : result.error);
  }

  _loadProject() {
    const result = this.storage.load();
    if (!result.success) { alert(result.error); return; }
    const { data } = result;
    this._projectName = data.projectName || '';
    this._metadata = {
      siteName: data.metadata?.siteName || '',
      operator: data.metadata?.operator || '',
      pipeType: data.metadata?.pipeType || ''
    };
    this.pointManager.loadPoints(data.points, data.calibration);
    alert('\u8AAD\u307F\u8FBC\u307F\u307E\u3057\u305F');
  }

  _newProject() {
    if (!confirm('\u73FE\u5728\u306E\u30C7\u30FC\u30BF\u3092\u7834\u68C4\u3057\u3066\u65B0\u898F\u30D7\u30ED\u30B8\u30A7\u30AF\u30C8\u3092\u4F5C\u6210\u3057\u307E\u3059\u304B\uFF1F')) return;
    this.pointManager.clear();
    this._projectName = '';
    this._metadata = { siteName: '', operator: '', pipeType: '' };
    this._lastHeading = 0;
  }
}

document.addEventListener('DOMContentLoaded', () => {
  window.app = new App();
  console.log('PipeScanner Web v2.7 initialized');
});
